...
bool Download::download(const char *url, const char*  fileDir, const char* filename, bool reload, void(*update)(unsigned long, unsigned long))
{
	...
	// 打开网络连接
		...
		//char szzz[2048] = {0x0};
		if (!HttpQueryInfoA(hIurl, HTTP_QUERY_FLAG_NUMBER | HTTP_QUERY_STATUS_CODE, &contentlen, &len, NULL))
		{
...
bool Download::download(const TCHAR * url, const TCHAR * filedir, const TCHAR * filename, bool reload, void(*update)(unsigned long, unsigned long))
{
	return false;
}
...
bool Download::download(const char *url, char* pbuf, int bufsize)
{
	...
	try
	{
		...
		// 打开网络连接
		HINTERNET hInet = InternetOpenA("downloader", INTERNET_OPEN_TYPE_DIRECT, NULL, NULL, 0);
		if (hInet == NULL)
			throw DLExc("Can't open connection");

		DWORD options = INTERNET_FLAG_NEED_FILE | INTERNET_FLAG_HYPERLINK | INTERNET_FLAG_RESYNCHRONIZE | INTERNET_FLAG_RELOAD;
		HINTERNET  hIurl = InternetOpenUrlA(hInet, url, NULL, -1, /*INTERNET_FLAG_NO_CACHE_WRITE*/options, 0);
		if (hIurl == NULL)
			throw DLExc("Can't open url");
		// 保证支持 HTTP/1.1 或者更高版本
		if (!httpverOK(hIurl))
		...
		if (!HttpQueryInfoA(hIurl, HTTP_QUERY_FLAG_NUMBER | HTTP_QUERY_STATUS_CODE, &content, &len, NULL))
		{
			throw DLExc("Can't get status code");
		}
...
bool  http_downloadfile(const char* url, const char*  fileDir, const char* filename, bool reload, download_callback callbk)
{
	try
	{
		...
		{
			OutputDebugStringA("Download Complete \n");
			return true;
		}
		else
			return false;
	...
	{
		OutputDebugStringA("\n");
		OutputDebugStringA(exc.geterr());
		OutputDebugStringA("Download Interrupted \n");
		return false;
