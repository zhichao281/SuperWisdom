...
TCHAR* PrintMD5(uchar md5Digest[16], TCHAR* pDestMd5, int iBufSz)
{
	...
	memset(pDestMd5, 0, iBufSz * sizeof(TCHAR));
	memset(chEach, 0, 8 * sizeof(TCHAR));

	for (nCount = 0; nCount < 16; nCount++)
	{
		_stprintf_s(chEach, 8, _T("%02x"), md5Digest[nCount]);
		_tcsncat_s(pDestMd5, iBufSz, chEach, 8);
	}
...
TCHAR* MD5String(TCHAR* szString, TCHAR* pDestMd5, int iBufSz)
{
	...
	CMd5 alg;
	char *pContent = new char[nLen+1];

#ifdef UNICODE
	WideCharToMultiByte(936, 0, szString, -1, pContent, nLen + 1, NULL, NULL);
#else
	strcpy_s(pContent, nLen+1, szString);
#endif

	alg.Update((unsigned char*)pContent, (unsigned int)nLen);
	alg.Finalize();

	delete []pContent;
	return PrintMD5(alg.Digest(), pDestMd5, iBufSz);
}
